...
- **M6.1.1** — Engine-cost instrumentation diagnosis & symmetrisation (code landed 2026-05-16 on branch `023-m6-1-1-engine-cost-instrumentation`; first live Phase 1 run completed 2026-05-16 returning mixed `channel_dependent_batching` + `drift_not_reproduced` under sequential dispatch; **blocked on M6.0a** for trustworthy classification): close a measurement gap M6.1 surfaced (chat_stream cells fire `engine_cost_drift_warning` with a consistent ~14-17% per-cohort engine_ttft spread; likely a REST-vs-gRPC measurement-window asymmetry, but possibly a real channel-dependent batching effect). Two-phase diagnose-then-fix-or-document. Drive with `python -m vllm_grpc_bench --m6_1_1-diagnose` (Phase 1 mini-sweep) then `--m6_1_1` (Phase 2 fix-or-document).
- **M6.0a** — Concurrent dispatch restoration (planned, blocks M6.1.1 closure): corrective methodology fix discovered during M6.1.1's first live run. M5.1 / M5.2 used `asyncio.gather`-based concurrent client-side dispatch at cell concurrency c; M6 / M6.1 / M6.1.1 silently dropped this and use sequential `await driver(...)` loops, making the cell's `concurrency` field a metadata tag rather than an in-flight parallelism control. M6 / M6.1 main verdicts are dispatch-robust and survive; the M6.1 per-cohort `engine_ttft_ms` drift sub-finding (and M6.1.1's classifier output, which presupposes concurrent dispatch for the `channel_dependent_batching` mechanism to be mechanistically possible) requires restored concurrency. Harness-only fix; re-runs M6.1.1's Phase 1 to get a real classification. No Modal compute beyond one ~30 min re-run.
