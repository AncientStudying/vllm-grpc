...
### M5 — Cross-Host Time-Axis Validation (active)
...
**Status (in flight 2026-05-11):** Harness wired on branch `017-m5-cross-host-validation` — `rtt_probe`, `modal_endpoint`, `m5_sweep`, `m5_supersede`, M5 reporter functions, and CLI flags (`--m5`, `--m5-modal-region`, `--m5-rtt-validity-threshold-ms`, `--m5-rtt-exercise-threshold-ms`, `--m5-warmup-n`, `--m5-skip-deploy`, `--m5-modal-endpoint`) are landed; Modal app at `scripts/python/modal_bench_grpc_server.py` exposes M3 servicers + Health + bearer-token interceptor on TLS-terminated `modal.forward()`. 271 harness tests green; the full operator-driven sweep against Modal is not yet executed (Modal-secrets-gated smoke at `tests/integration/test_m5_modal_smoke.py`).
