...
* Four primary sections (Production latency budget / TPOT curves / Engine-cost
  decomposition curves / Protocol crossover threshold).
* Six auxiliary subsections (KV-cache pressure / Null anchor validation /
  Anchor latency trajectory / Failure summary / Sweep wall-clock timeline /
  Method-Background reciprocal cross-reference).
* Four publish-blocking-eligible sweep-level integrity warning headers
  rendered as leading callouts (``null_anchor_drift``,
  ``failure_summary_threshold``, ``cohort_csp_mismatch``,
  ``intra_sweep_latency_drift``) plus the soft
  ``iteration_discipline_broken`` diagnostic.
* Per-row ``prompt_source`` + ``measurement_regime`` + ``prompt_corpus_idx``
  columns for the three-regime audit trail (round-5 FR-034 / FR-035).
* Validate-mode rendering: interior caps marked ``not_validated``; crossover
  axis-restricted callout + coarse vocabulary; wall-clock timeline OMITTED
  if total sweep < 8h.

The reporter is the integration point both ``--m6_2`` and ``--m6_2-validate``
flow through after the sweep completes. US2 (crossover) and US3 (sub-probe /
KV-pressure) fill the corresponding sections; US1 ships the production budget
+ null-anchor + failure summary + wall-clock + anchor trajectory rendering.

``schema_version`` stays at ``"m6_1_1.v1"`` per FR-011 (additive evolution).
"""
...
def _iter_cell_ids() -> list[str]:
    ...
    return [f"{path}_c{c}" for path, _h, c in M6_1_CELLS]
...
def fill_validate_mode_placeholders(
    ...
    *,
    ...
    block_start_utc: str,
    block_end_utc: str,
) -> dict[str, dict[M6_1_2CohortKind, dict[int, M6_2MeasurementPoint]]]:
    ...
    if sweep_mode != "validate":
        return per_cell
    out: dict[str, dict[M6_1_2CohortKind, dict[int, M6_2MeasurementPoint]]] = {
