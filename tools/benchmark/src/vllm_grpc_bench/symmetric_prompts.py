"""M5.2-era cohort-prompt symmetry helper, relocated as a cross-milestone
shared module per M6.1.3 FR-019.

The implementation is preserved verbatim from ``m5_2_symmetry.py``; the
relocation makes the helper the canonical import target for any future
milestone that needs cohort-prompt symmetry checks (M5.2, M6.1.3, M6.2,
M7, M8). ``m5_2_symmetry.py`` becomes a one-line re-export shim so M5.2's
historical sweep + tests continue to resolve their imports unchanged
(per R-6).
