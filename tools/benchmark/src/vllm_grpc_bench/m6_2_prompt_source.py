"""M6.2 — three-regime prompt-source resolver (round-5 FR-034 / FR-035).

Round-5 of `/speckit-clarify` resolved the prompt-source gap: M6.x's harness
defaults are synthetic seed-derived prompts (chat) and random tensors
(embed), which silently break M6.2's `max_tokens=2048` row + the FR-017a
wall-clock-ratio inference. M6.2 splits the matrix into three regimes:

- Null-anchor cells (``max_tokens ∈ {10, 50}``) — keep the M6.1.x synthetic
  prompts to preserve byte-comparability with M6.1.3's published anchors.
- Interior-cap cells (``max_tokens ∈ {256, 512, 1024, 2048}``) — switch to
  ShareGPT chat corpus (or the new Qwen3-8B prompt-embedding corpus for the
  embed cohort), giving production-realistic prompt distributions.
...
This module pins the regime resolution logic + corpus loaders + SHA validation
+ ``ignore_eos`` plumbing. See ``contracts/prompt-source.md`` for the full
regime table.
"""
    ...
    if regime == "null_anchor" and not ignore_eos:
        if cell_type == "chat_stream":
            ...
            return ResolvedBlockInputs(
                prompt_text=_build_chat_prompt(seed),
