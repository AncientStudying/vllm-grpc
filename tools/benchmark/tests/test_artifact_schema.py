...
Exercises :mod:`reporter` directly: renders ``SweepArtifact``
...
from vllm_grpc_bench.types import CELLS, COHORTS, TopologyPath
...
def _build_artifact(
    *,
    ...
    anchor_trajectories: dict[str, AnchorLatencyTrajectory] | None = None,
    null_anchor_validation: list[NullAnchor] | None = None,
    iteration_discipline_verified: bool = True,
    clock_anomaly_rows: set[tuple[str, str, int]] | None = None,
) -> SweepArtifact:
    failures = failures or {}
    clock_anomaly_rows = clock_anomaly_rows or set()
    per_cell: dict[str, dict[str, dict[int, MeasurementPoint]]] = {}
    for path, _hidden, concurrency in CELLS:
    ...
    artifact = SweepArtifact(
        ...
        network_paths=network_paths or {cohort: [] for cohort in COHORTS},  # type: ignore[arg-type]
...
def test_publish_artifact_has_144_rows() -> None:
    ...
    artifact = _build_artifact(sweep_mode="publish", measured_axis=MAX_TOKENS_AXIS)
    rows = [
        point
...
def test_validate_artifact_has_72_measured_plus_72_placeholders() -> None:
    artifact = _build_artifact(sweep_mode="validate", measured_axis=VALIDATE_MAX_TOKENS_AXIS)
    rows = [
        point
        for per_cohort in artifact.per_cell.values()
        for per_cap in per_cohort.values()
...
def test_run_meta_has_all_additive_fields() -> None:
    ...
    assert rm["validate_axis_subset"] == list(VALIDATE_MAX_TOKENS_AXIS)
...
def test_iteration_discipline_broken_fires_soft_diagnostic() -> None:
    artifact = _build_artifact(
        sweep_mode="validate",
        measured_axis=VALIDATE_MAX_TOKENS_AXIS,
...
def test_cohort_csp_mismatch_fires_on_consecutive_provider_change() -> None:
    network_paths: dict[str, list[Any]] = {cohort: [] for cohort in COHORTS}
...
def test_intra_sweep_latency_drift_fires_at_two_of_four_cohorts() -> None:
    ...
    trajectories: dict[str, AnchorLatencyTrajectory] = {
        "default_grpc": AnchorLatencyTrajectory(
        ...
        "rest_https_edge": AnchorLatencyTrajectory(
            cohort="rest_https_edge",  # type: ignore[arg-type]
            snapshots=snapshots_drifted,
            max_minus_min_wall_p50_ms=50.0,
            latency_drift_warning=True,
        ),
        "rest_plain_tcp": AnchorLatencyTrajectory(
        ...
        "tuned_grpc_multiplexed": AnchorLatencyTrajectory(
