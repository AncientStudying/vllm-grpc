...
from vllm_grpc_bench.m6_2_types import M6_2AnchorLatencySnapshot


def _stub_rpc_driver(values_by_cohort: dict[str, list[float]]):
    def driver(
        *,
...
class TestAnchorBlock:
    def test_per_cohort_snapshot_built_from_rpc_driver(self) -> None:
        values = {c: [10.0, 12.0, 14.0, 16.0, 18.0] for c in M6_1_2_COHORTS}
        snapshots = compute_anchor_block(
    ...
    def test_empty_cohort_skipped(self) -> None:
        values = {c: [] for c in M6_1_2_COHORTS}
...
def _make_trajectory(cohort: str, *, drift: bool):
