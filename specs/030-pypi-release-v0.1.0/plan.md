...
| **II. Library Dependency, Not Fork** | ✅ PASS | vLLM stays a peer/documented prerequisite (FR-005); not added to `install_requires`, not vendored. The V0→V1 remediation (FR-022) uses vLLM's *public* `AsyncLLM` API — adapter code here, no patched vLLM copy. |
