...
- [X] T001 Add `twine` to the `dev` dependency group in `pyproject.toml` (root) so metadata
  validation (`uv run twine check` / `uvx twine`) is available; run `uv sync --all-packages` to
  resolve it.
- [X] T002 Capture the green baseline: run `make proto` then
...
- [X] T002a Scan **all four** published packages' own source for deprecated-API usage to build the
...
- [X] T002b Verify distribution-name availability **read-only, no upload** (Edge case "Name already
...
- [X] T003 Fill complete `[project]` metadata in `packages/gen/pyproject.toml` per
...
- [X] T004 Add the build-time stub generation hook for gen (FR-007a): create
...
- [X] T005 [P] Author `packages/gen/README.md` (long-description): purpose (generated protobuf/gRPC
...
- [X] T006 Build gen and validate: `uv build --package vllm-grpc-gen`; assert the wheel AND sdist
...
- [X] T007 Install gen alone into a fresh isolated env (`uv venv` + `uv pip install --no-project`)
...
- [X] T008 [P] [US1] Fill complete metadata in `packages/client/pyproject.toml` per
...
- [X] T009 [P] [US1] Author `packages/client/README.md`: purpose (lean async gRPC client),
  `pip install vllm-grpc-client`, a copy-pasteable usage snippet exercising the client, repo +
  changelog links (FR-002).
- [X] T010 [US1] Build the client and validate metadata: `uv build --package vllm-grpc-client`;
...
- [X] T011 [US1] Clean-env install + leanness check: install the client wheel into a fresh venv with
...
- [X] T012 [P] [US2] Fill complete metadata in `packages/proxy/pyproject.toml` per
  ...
  (FR-004).
- [X] T013 [US2] Add a `main()` console entry to `packages/proxy/src/vllm_grpc_proxy/main.py` that
...
- [X] T014 [P] [US2] Author `packages/proxy/README.md`: purpose (REST proxy), `pip install
  vllm-grpc-proxy`, how to run the `vllm-grpc-proxy` console script, a usage snippet, repo +
  changelog links (FR-002).
- [X] T015 [P] [US2] Fill complete metadata in `packages/frontend/pyproject.toml` per
...
- [X] T016 [US2] Remediate the deprecated V0 engine path in
...
- [X] T017 [P] [US2] Author `packages/frontend/README.md`: purpose (gRPC frontend), `pip install
  ...
  (FR-002/FR-005a).
- [X] T018 [US2] Build + validate the server packages: `uv build --package vllm-grpc-proxy` and
...
- [X] T019 [US2] Clean-env install + console-script smoke (no GPU/model): install proxy and frontend
...
- [X] T019a [US2] Run the Constitution §IV merge-gate checks after the source edits in T013
...
- [X] T020 [US3] Author `.github/workflows/release.yml` per `contracts/release-workflow.md`:
...
- [X] T021 [US3] Statically validate the workflow with zero execution: run
...
- [X] T022 [P] [US4] Update the root `README.md` install matrix: `pip install vllm-grpc-client` for
...
- [X] T023 [P] [US4] Update `CONTRIBUTING.md` with the release procedure: version bump (all four in
  lockstep) → tag (`v*`) → pipeline publish (test index then gated real index) → release-notes draft
  (FR-017).
- [X] T024 [P] [US4] Create `CHANGELOG.md` in Keep-a-Changelog format with a `v0.1.0` entry
  summarizing this release; this is the target each package's `changelog` project URL points to
  (FR-018a).
- [X] T025 [P] [US4] Create `docs/RELEASES.md` listing the prior codebase-state tags (`v0.0.0`,
  `v0.0.1`) and the `v0.1.0` entry as human-readable release notes (FR-018).
- [X] T026 [US4] Verify every package's `changelog` `[project.urls]` value resolves to the
...
- [X] T026a Regenerate and commit `uv.lock` after the dependency-declaration changes (T003 gen
...
- [X] T026b Commit the v0.1.0 packaging ADR `docs/decisions/0009-pypi-packaging-v0.1.0.md` (the
...
- [X] T027 [P] Simplify `README.md` and `ANALYSIS.md` section-by-section per FR-025: rewrite dense
...
- [X] T028 [P] Consistency review across `README.md`, `ANALYSIS.md`, `CONTRIBUTING.md` (FR-026):
...
- [X] T029 Run the full test suite green at the bumped floors: `make proto` then
  `uv run pytest packages/proxy/tests packages/frontend/tests packages/client/tests tools/benchmark/tests -q`
  — confirm parity with the T002 baseline (SC-009).
- [X] T030 Build all four and final-validate: `for p in gen proxy frontend client; do uv build
  ...
  (FR-006a).
- [X] T030a Assert metadata **completeness** across all four built distributions (SC-006 — beyond
...
- [X] T031 Execute `quickstart.md` end-to-end (steps 1–8) and confirm the no-upload invariant: no
  `twine upload` / `uv publish` / publish job ran, and no distribution name was claimed on any
  external index (FR-014a/FR-020/SC-008).
- [X] T032 Run `graphify update .` to refresh the code graph after the source edits in
