...
| **II. Library Dependency, Not Fork** | **PASS** | M6 uses `vllm` as an ordinary dependency. No vLLM source code is copied or modified. Real engine launch reuses the frontend's prebuilt `enable_prompt_embeds=True` AsyncEngineArgs flag. AsyncLLM's lack of native per-request timing surface (Research R-3) is handled by adapter code in our gRPC frontend, not by patching vLLM. |
...
└── main.py                     # UNCHANGED (prebuilt enable_prompt_embeds=True support)
