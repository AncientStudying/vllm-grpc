...
- **Real-engine path already exists.** The project's gRPC frontend is already capable of launching real `AsyncLLM` (the frontend already exposes `enable_prompt_embeds=True` via `AsyncEngineArgs`); M6 inherits that capability and configures it for the chosen real model.
