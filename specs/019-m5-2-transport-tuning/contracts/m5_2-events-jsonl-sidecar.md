...
| `phase` | string | `warmup` or `measurement`. Warmup records are persisted (for audit) but excluded from aggregates per FR-011. |
...
| `response_body_bytes` | int | Number of bytes in the response body (sum across SSE frames for `chat_stream`; full body for `embed`). |
