...
- [x] T001 Create `scripts/setup/` directory and empty placeholder at `scripts/setup/phase2-env.sh` (will be filled in Phase 5) at `scripts/setup/phase2-env.sh`
- [x] T002 [P] Create `docs/decisions/` directory and ADR skeleton at `docs/decisions/0001-prompt-embeds-environment.md` — include sections: vLLM Version, Candidate A (M2 MPS), Candidate B (M2 CPU), Candidate C (Cloud GPU), Decision, Rationale, Rejected Alternatives at `docs/decisions/0001-prompt-embeds-environment.md`
- [x] T003 Install vLLM in a scratch environment (`uv run --with vllm python -c "import vllm; print(vllm.__version__)"`); run `uv run --with vllm vllm serve --help | grep -i "prompt.embed\|v1.engine\|v0\|enable"` and record the exact flag names in the ADR's vLLM Version section at `docs/decisions/0001-prompt-embeds-environment.md`
...
- [x] T004 Implement `scripts/python/verify_prompt_embeds.py`: accepts `--base-url`, `--model`, `--seq-len` (default 8), `--max-tokens` (default 50) CLI arguments; constructs a float32 numpy array of shape `[seq_len, 1024]`; base64-encodes it; sends `POST /v1/completions` with `extra_body: {"prompt_embeds": <b64>}`; prints `[OK] Server responded in X.XXs` and `[OK] prompt_embeds accepted` on success, or a descriptive `[FAIL]` message on any error; exits 0 on success, 1 on failure at `scripts/python/verify_prompt_embeds.py`
- [x] T005 Add `py.typed` marker, full type annotations, and `__main__` guard to `verify_prompt_embeds.py` so it passes `mypy --strict` with zero errors at `scripts/python/verify_prompt_embeds.py`
- [x] T006 Run `ruff check scripts/python/verify_prompt_embeds.py` and `ruff format --check scripts/python/verify_prompt_embeds.py`; fix any violations at `scripts/python/verify_prompt_embeds.py`

**Checkpoint**: Verification script exists and passes `make check`. Experiment work can begin.

---

## Phase 2b: Version Alignment (vLLM 0.19.0 + vllm-metal 0.2.0)

**Purpose**: The original investigation used vLLM 0.11.0 and did not test vllm-metal empirically. These tasks update the ADR's version section and add empirical evidence for all three candidates using the current ecosystem versions.

**Why added**: vLLM 0.19.0 exists in the pre-installed `~/.venv-vllm-metal` environment (alongside vllm-metal 0.2.0). Architectural decisions must reflect the current ecosystem, not a point-in-time snapshot. The constitution requires empirical evidence — source code inspection alone does not satisfy FR-002.

- [x] T020 Add `[dependency-groups] investigation` to `pyproject.toml` containing the vllm-metal 0.2.0 GitHub wheel; run `uv sync --group investigation` to verify clean install; verify `uv run --group investigation --with vllm` activates `MetalPlatform` (confirmed: vllm 0.11.0 + vllm-metal 0.2.0 work together; vllm 0.20.0 cannot install on macOS due to CUDA-only `nvidia-cudnn-frontend` dep) at `pyproject.toml`
- [x] T017 Update the ADR vLLM Version section to reflect findings from T020: vllm 0.11.0 (macOS) + vllm-metal 0.2.0; confirm `--enable-prompt-embeds` flag; note vllm 0.20.0 macOS install blocker; record wire format at `docs/decisions/0001-prompt-embeds-environment.md`
- [x] T018 [US1] Empirically test Candidate A with vllm-metal 0.2.0 + vllm 0.11.0: start `uv run --group investigation --with vllm vllm serve Qwen/Qwen3-0.6B --enable-prompt-embeds --max-model-len 256 --port 9002`; run the verification script against it; record the actual HTTP response or error (not source-code inference) in the ADR Candidate A section at `docs/decisions/0001-prompt-embeds-environment.md`
- [x] T019 [US1] Empirically test Candidate B with vllm 0.11.0 CPU: start `VLLM_PLUGINS="" uv run --group investigation --with vllm vllm serve Qwen/Qwen3-0.6B --enable-prompt-embeds --max-model-len 256 --port 9003`; run the verification script; record the actual HTTP response or error in the ADR Candidate B section at `docs/decisions/0001-prompt-embeds-environment.md`

**Checkpoint**: ADR Candidate A and B sections contain empirical HTTP-level evidence (not just source inspection) from vLLM 0.20.0 + vllm-metal 0.2.0.
...
- [x] T014 Run `make check` (ruff + mypy --strict) across all new and modified files in `scripts/`; fix any lint, format, or type errors at `scripts/python/verify_prompt_embeds.py`, `scripts/setup/phase2-env.sh`
- [x] T015 [P] Follow `specs/002-phase2-prompt-embeds/quickstart.md` step by step on a clean shell; confirm each step produces the documented output; update quickstart.md if any step is inaccurate at `specs/002-phase2-prompt-embeds/quickstart.md`
...
- **Version Alignment (Phase 2b)**: Depends on Phase 2 — updates ADR with vLLM 0.19.0 empirical evidence; T007/T008 results are superseded by T018/T019
- **US1 (Phase 3)**: Depends on Phase 2b — T009/T010 must use vLLM 0.19.0 findings
