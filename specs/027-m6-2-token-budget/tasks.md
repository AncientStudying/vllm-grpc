...
- [ ] T073 Fix the anchor-block over-firing bug in `m6_2_sweep.should_run_anchor_at` (`m6_2_sweep.py:417`). The `if sweep_hour_mark <= epsilon_hours: return True` branch was intended to catch "this is the sweep-start mark (0.0)", but the inclusive `<=` against the 0.05 h epsilon causes the orchestrator to re-fire the anchor block after EVERY (cell, max_tokens) tuple whose end mark lands in the first 3 minutes. **Evidence** from the 2026-05-24 13:44 UTC validate run (surfaced by Phase 9 / T067 progress instrumentation): `ANCHOR_START sweep_hour_mark=0.00` (legitimate sweep-start anchor) followed at 13:46:05 by a duplicate `ANCHOR_START sweep_hour_mark=0.02` immediately after tuple 1 (embed_c1 × max_tokens=10) completed. **Impact**: each stray anchor costs ~36 s wall-clock + ~$0.015 Modal at $1.51/h; the validate sweep sees 1–3 stray anchors in the first 3 minutes (~30 s–2 min extra wall-clock, ~$0.05). **Downstream analysis is still correct** — the round-8 C1 warmup-suppression amendment (T061) drops snapshots with `sweep_hour_mark < 0.05` from the trajectory spread computation, so the stray snapshots are properly excluded from the drift verdict. The bug is wasted compute, not wrong data. **Fix**: tighten the start-guard from `<= epsilon_hours` to `<= 0.001` (or a dedicated `START_GUARD_HOURS` constant). The sweep-start anchor is already captured BEFORE the main loop begins (line 559-573 region), so this branch only needs to handle the "exactly t=0" case — no 3-minute tolerance is required. Add a regression unit test in `test_m6_2_anchor_trajectory.py::TestCadenceRule` asserting that `should_run_anchor_at(0.02, "validate")` returns False. ~10 LOC.
- [ ] T074 Implement Modal-preemption mid-sweep recovery (closes the implementation gap behind FR-026's "auto-resume handler" promise; surfaced by the 2026-05-24 13:44 UTC validate run where Modal preempted the worker at 14:58:44 UTC after Tuple 6 of 18, killing 48 of 66 remaining blocks because the orchestrator had no way to refresh the dead endpoint handles before the new worker came back online at 15:01:35 UTC). **Symptom signature**: every dispatcher call returns `StatusCode.UNAVAILABLE failed to connect to all addresses; last error: UNKNOWN: ipv4:<old_modal_ip>:<port>: F` (gRPC) or `ConnectError: All connection attempts failed` / `ReadError` / `nodename nor servname provided, or not known` (REST). The dispatcher's cached `M6EndpointHandles` point at the OLD container's IP/port pair; Modal's preemption-restart contract gives the new worker a fresh IP. **Implementation plan**:
  - **T074a — failure classifier**: add `is_modal_endpoint_death(exc: BaseException) -> bool` to `m6_2_validate.py` recognising the four symptom-signature error shapes. Distinct from `is_transient_modal_error` (which classifies retry-eligible single-RPC errors). ~15 LOC.
  - **T074b — block-level recovery wrapper**: extend `m6_2_validate.build_modal_block_dispatcher` so that when ALL n RPCs in a block fail with `is_modal_endpoint_death`, the dispatcher (1) emits a new `[m6_2 PREEMPTION_DETECTED]` progress event with the dead endpoint + sweep-hour mark, (2) tears down the current driver via the `provide_m6_2_rpc_driver` async-context-manager, (3) polls `provide_m6_endpoint` until the new Modal worker accepts a smoke RPC (expected ~2-3 min based on the 14:59:41→15:01:35 vLLM init sequence observed in the failed sweep), (4) emits `[m6_2 PREEMPTION_RECOVERED]` with the new endpoint + recovery wall-clock, (5) retries the current block against the refreshed driver. Anchor dispatcher gets the parallel treatment. ~150 LOC across `m6_2_validate.py` + tests.
  - **T074c — preemption-counter integration**: thread the FR-026 `M6.2_PREEMPTION_RECURRENCE_THRESHOLD = 2` gate through the block recovery wrapper. Each successful recovery increments a per-sweep counter; exceeding the threshold causes the orchestrator to emit `[m6_2 PREEMPTION_BUDGET_EXHAUSTED]` and abort. Persisted in `run_meta.preemption_events: int` (additive schema field per FR-011 strict-superset). ~25 LOC + an additive top-level schema test in `test_m6_2_artifact_schema.py`.
  - **T074d — Modal endpoint provider**: confirm `modal_endpoint.provide_m6_endpoint` supports re-entry against the same Modal app handle (the preemption-restart re-uses the same `ap-fnef5BeZFrHBCHeiYxhz4R` app ID). If it doesn't, add a `refresh_endpoint_after_preemption(app_handle)` helper that queries the live Modal app for the current container's IP/port and rebuilds the `M6EndpointHandles` dataclass. ~30 LOC.
  - **T074e — tests**: add `tools/benchmark/tests/test_m6_2_preemption_recovery.py` with: (i) classifier-positive tests for each of the four symptom-signature error shapes; (ii) classifier-negative tests for unrelated errors (gRPC RESOURCE_EXHAUSTED, REST 502, generic `RuntimeError`); (iii) block-recovery happy-path test using a fake driver that fails the first n RPCs with UNAVAILABLE then succeeds; (iv) anchor-recovery happy-path; (v) preemption-budget-exhausted test that asserts the orchestrator aborts after `M6.2_PREEMPTION_RECURRENCE_THRESHOLD` recoveries. ~250 LOC.
  - **T074f — sweep-progress event additions**: extend `m6_2_sweep._progress` event vocabulary with `PREEMPTION_DETECTED` and `PREEMPTION_RECOVERED` + add the new tags to the monitor script's `parse_progress_events` regex tolerance (already permissive, no code change required). Update `monitor_m6_2_sweep.derive_sweep_state` to surface a `preemption_events` count in the status line. ~25 LOC + monitor test.
  - **T074g — quickstart documentation**: add a "Preemption recovery" subsection to `specs/027-m6-2-token-budget/quickstart.md` explaining the expected event sequence (`PREEMPTION_DETECTED` → ~2-3 min pause → `PREEMPTION_RECOVERED` → block resumes) and the FR-026 budget-exhaustion abort path. ~30 LOC.
  - **Total scope**: ~525 LOC across 4 source files + 2 test files + 1 doc file. Estimated 2-3 hour implementation cycle including the local lint chain pass. **Priority justification**: the publish sweep is ~13 h wall-clock and ~$20 Modal spend; a single mid-sweep preemption today would burn most of the run and force a complete restart. Recovery shortens the blast radius from "lose the whole sweep" to "lose ~3 min of wall-clock + retry the current block." A second preemption within the same sweep still aborts cleanly per FR-026's 2-recurrence threshold rather than spinning forever.
