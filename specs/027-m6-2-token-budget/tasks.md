...
- [ ] T069 [P] Document the progress-event format + monitor script in `specs/027-m6-2-token-budget/quickstart.md`: add a "Monitoring an in-flight sweep" subsection between Stage 1 (validate) and Stage 2. Cover: how to tee the sweep stderr to a log file (`python -m vllm_grpc_bench --m6_2-validate ... 2> sweep.log`), how to invoke the monitor (`python scripts/python/monitor_m6_2_sweep.py --log sweep.log --pid $!`), one-shot vs continuous modes, example output line. ~30 LOC.
