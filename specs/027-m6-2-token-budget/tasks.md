...
- [ ] T066 Re-calibrate FR-022 (validate-mode wall-clock budget, currently 2.3–2.5h) and FR-023 (publish-mode wall-clock budget, currently 20–48h depending on n) against the post-fix serial-dispatch runtime. Both numbers were implicitly calibrated against the pre-fix bug where c=1 cells were accidentally batched n-way concurrent, making them ~20× faster than the now-correct serial dispatch. Post-fix data point: the 2026-05-23 evening validate re-run (commit `f3e0989`) took ~5h vs the spec's 2.3–2.5h estimate — about 2× the budget. Update FR-022 to reflect actual post-fix wall-clock (with the c=1 × mt=2048 blocks dominating: 20 RPCs × ~60–160s serial per cohort × 4 cohorts × 2 cell-types ≈ 3.5–4.5h on the c=1 × mt=2048 tuple alone), and re-derive FR-023 publish-mode projections at each candidate n (50, 75, 100, 150) accordingly. Also revise SC-002 Modal-spend estimates if wall-clock change materially affects A10G $/hr × wall-clock cost. ~50 LOC across spec.md, plan.md narrative, and contracts/cli.md if needed. Depends on the in-flight validate sweep (PID 69547) producing its actual wall-clock data point.
