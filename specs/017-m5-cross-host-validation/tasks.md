...
- [ ] T041 [US2] Extend `scripts/python/modal_bench_grpc_server.py` to support schema-candidate registration: `serve_bench` accepts an optional `schema_candidate: str | None` argument; when set, the Modal app registers the matching candidate servicer (`packed_token_ids`, `oneof_flattened_input`, `chunk_granularity`) on a *separate gRPC port* from the production servicers (e.g., 50052), publishes that port's tunnel URL via `modal.Dict("vllm-grpc-bench-mock-handshake")` under key `endpoint_candidate`, so the harness can switch endpoints per-cohort without redeploying (depends on T022)
...
- [ ] T050 [P] [US3] Extend `tools/benchmark/src/vllm_grpc_bench/reporter.py` with the "Supersedes M4" table section: emit JSON `supersedes_m4` array (top-level); emit Markdown table with verdict-changed rows visually distinguished and sorted-first per SC-004 (depends on T044 reporter scaffold)
...
Phase 3 (US1, P1, MVP):              Phase 4 (US2, P2):                  Phase 5 (US3, P3):
  T012..T020 [tests, mostly P]         T035..T038 [tests, mostly P]        T045..T047 [tests, mostly P]
       │                                    │                                    │
       ▼                                    ▼                                    ▼
  T021..T034 (impl, mixed P)            T039..T044 (impl, mixed P)          T048..T050 (impl)
       │                                    │                                    │
       ▼                                    ▼                                    ▼
...
**Critical-path execution order**: T001 → T002/T003 (parallel) → T004 → T005 → T006 → T007 → T008 → US1 (T012..T034) → US2 (T035..T044) → US3 (T045..T050) → Polish (T051..T057).
...
- Phase 5 (US3): T045 || T046 || T047 (all tests, different files). T050 parallel to T048..T049.
