...
- [ ] T015 [P] [US1] Create `tools/benchmark/src/vllm_grpc_bench/m4_sweep.py` skeleton with `run_m4_sweep(config: M4SweepConfig) -> Run` signature, M4SweepConfig import, and stub helpers (`measure_shared_baseline`, `measure_candidate`, `validate_run`)
- [ ] T016 [US1] Implement shared-baseline orchestration in `tools/benchmark/src/vllm_grpc_bench/m4_sweep.py`: measure the M1_BASELINE cohort once per path at n ≥ `config.baseline_n`; record cohort id + metadata; populate `Run.shared_baseline_cohort_ids` and tag the cohorts with `is_baseline=True, baseline_role="m1_shared"` (FR-002 / R-2) (depends on T015)
...
- [ ] T020 [US1] Wire TTFT as the primary chat_stream verdict metric in `m4_sweep.build_recommendations` (consumes `tools/benchmark/src/vllm_grpc_bench/ttft.py` from T014); total wall-clock is recorded as a secondary diagnostic (FR-003 / R-10) (depends on T014, T019)
...
- [ ] T032 [P] [US2] Create `tools/benchmark/src/vllm_grpc_bench/m4_supersede.py`: read `docs/benchmarks/m3-channel-tuning-time.json`; for each M3 cohort with verdict `noise_bounded`, generate a `SupersessionEntry(m3_cell_id, m3_verdict, m4_cell_id, m4_verdict, rationale)` matching against the M4 sweep's cohorts on `(path, hidden_size, axis, config_name)` (FR-007 / FR-009)
...
- US3 tests (T036–T039) live in different files → all [P]-eligible.
