...
## R-11 — Baseline-variance acceptance threshold (FR-005)

**Decision**: Default acceptance threshold is **≤ 5% within-cohort coefficient of variation** (standard deviation / mean) on the time metric for each shared M1_BASELINE cohort. A cohort exceeding the threshold causes the harness to abort with a non-zero exit and a diagnostic message naming the cohort id and observed CV. The threshold is configurable via CLI flag `--baseline-cv-max=<float>` so operators can loosen it on noisy hosts (with a warning, since the verdict CI rule's effective floor scales with baseline CV).

**Rationale**: 5% is calibrated against M3's observed cross-batch baseline drift (~13% per-axis fresh baselines, per `specs/015-m3-protobuf-grpc-tuning/research.md` R-12). Capping CV at 5% bounds the largest expected channel-tuning effect size (~3–5% TTFT) above the baseline noise floor with margin. Failing loud rather than silently emitting verdicts is preferred (Constitution V): a poisoned baseline produces meaningless verdicts that look like real ones.

**Alternatives considered**:
- Stricter threshold (≤ 2%): rejected — would frequently fail-out runs on commodity macOS hosts where background load variance approaches that floor.
- Looser threshold (≤ 10%): rejected — falls inside the channel-tuning effect-size band, defeating the FR-002 rationale.
- Replace fail-loud with a "low-confidence" flag: rejected — a poisoned baseline doesn't yield "low-confidence verdicts," it yields incorrect verdicts. The right action is to re-run.
