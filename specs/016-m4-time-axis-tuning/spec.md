...
- **SC-004**: No verdict in the M4 report cites cross-batch baseline drift as the reason for an `no_winner` decision. The shared-baseline cohort's reproducibility is demonstrated within the report itself (e.g., by reporting the cohort's CIs and within-cohort variance against the FR-005 acceptance threshold).
