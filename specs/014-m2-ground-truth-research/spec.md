...
- Q: How should the project ensure vLLM is lockfile-pinned so `cross-repo.json` includes vLLM internals (today vLLM is loaded via `--with vllm`, so neither `uv.lock` nor `pyproject.toml` pins it)? → A: Pin vLLM (and grpcio, already pinned transitively) in `pyproject.toml` — added under a dependency-group declared for graph targets, with environment markers (e.g., `sys_platform == 'linux'`) where the package has CUDA-only wheels — and run `uv lock` so the version lands in `uv.lock` for the refresh skill to resolve.
- Q: Should `make check` regression and the cadence-table trigger-row presence be left to implicit coverage, or get explicit verification tasks? → A: Add explicit verification tasks: (1) a polish-phase task that exercises the cadence end-to-end (commit → hook fires; lockfile bump → skill drift-rebuilds) addressing SC-005; (2) fold an "all 5 trigger rows present and named" check into the cadence-table edit task addressing FR-007.
...
- vLLM has CUDA-only wheels and cannot be installed on macOS without source builds; the pinning required by FR-019c MUST therefore use a `sys_platform == 'linux'` (or equivalent) marker so `uv lock` resolves the version into `uv.lock` without breaking macOS dev environments. The refresh skill reads the pinned version from `uv.lock` regardless of platform.
...
- **FR-019c**: `pyproject.toml` MUST pin vLLM and grpcio at versions the refresh skill can resolve from `uv.lock` (e.g., under a dedicated `[dependency-groups]` graph-targets group). Where a target has wheels for only one platform (vLLM's CUDA-only wheels on Linux), the pin MUST use environment markers (e.g., `sys_platform == 'linux'`) so `uv lock` succeeds on every dev platform while still committing the version to `uv.lock`. This requirement exists because `cross-repo.json` is the substrate for SC-002 vLLM-internals queries; an unpinned upstream silently degrades the merged graph and makes SC-002 untestable.
