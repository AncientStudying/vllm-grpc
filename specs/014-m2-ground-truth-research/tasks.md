...
- [X] T009 [US1] One-time setup: run the four commands from `ground-truth-workflow-for-associated-projects.md` §Setup to produce the upstream graphs and the merged artifact — `graphify clone https://github.com/vllm-project/vllm`, `graphify clone https://github.com/grpc/grpc`, `graphify .`, `graphify merge-graphs ./graphify-out/graph.json ~/.graphify/repos/vllm-project/vllm/graphify-out/graph.json ~/.graphify/repos/grpc/grpc/graphify-out/graph.json --out cross-repo.json`. Confirms US1 acceptance scenarios 1 and 2; produces `cross-repo.json` at the documented path. (Skill-based path is delivered in US3.) **Closed (2026-05-09):** the literal four-command sequence is **superseded** by the `/ground-truth-refresh` skill (T020–T022) and the FR-019b reconciliation (T028). The exact commands listed here predate the `--branch v<version>` cold-build pattern and the targeted `src/python/grpcio/` indexing decision; running them as written would produce a less-correct graph (HEAD-of-default-branch instead of the lockfile-pinned tag, and a 55K-node grpcio mass that crowds out the Python wrapper layer). `cross-repo.json` IS produced at the documented path — by the skill — and verification of US1 acceptance scenarios 1+2 is recorded under T010. T009 is closed with this note rather than re-executed.
...
- [X] T025 [US3] Verify US3 acceptance scenario 1 from `quickstart.md` §Step 4: invoke `/ground-truth-refresh` from cold; confirm per-step progress, that `cross-repo.json` is produced, and that the final status line names the absolute path. Anchors FR-018, SC-008. **Depends on T003** (skill resolves versions from `uv.lock`). **Result (2026-05-09):** verified during this session. The skill was invoked from cold after T008, producing `cross-repo.json` at `/Users/bsansom/projects/vllm-grpc/cross-repo.json` (announced as the absolute path in the final status block, per FR-018). All seven steps surfaced as discrete entries on stdout. Subsequent runs (the targeted-grpcio retest after the SKILL.md re-scope, and the FR-019b reconciliation in T028) re-exercised the same cold-path mechanics.

- [X] T026 [US3] Verify US3 acceptance scenario 2 from `quickstart.md` §"Re-running the refresh skill": re-invoke on an unchanged tree; confirm the skipped steps are reported and total runtime is substantially faster than T025's cold run. Anchors FR-019, SC-009. **Result (2026-05-09):** re-invoked the skill against the post-T028 state (sidecars `0.20.1` / `1.80.0` matching `uv.lock`). All seven steps surfaced cleanly: cwd OK, graphify on PATH, lockfile resolves to `vLLM @ 0.20.1` + `grpcio @ 1.80.0`, both upstream sidecars matched the lockfile → both correctly emitted `cache current — skipped`. `graphify update .` rebuilt the project graph from SHA256 cache in **1 s**. `graphify merge-graphs` produced `cross-repo.json` (54,709 nodes / 261,726 edges) in **3 s**. **Total idempotent runtime: ~5 s** versus the cold-first-build runtime of several minutes. SC-009 ("substantially faster than cold") satisfied with a >50× speedup. FR-019 cache-honoring behavior confirmed end-to-end.

- [X] T027 [US3] Verify US3 acceptance scenarios 3 from `quickstart.md` §Failure recoveries: temporarily move the `graphify` binary off PATH (or unset PATH in a subshell) and invoke the skill; confirm the actionable install message is surfaced rather than a raw `command not found`. Anchors FR-020. **Result (2026-05-09):** `env -i PATH=/usr/bin:/bin sh -c "command -v graphify >/dev/null 2>&1; echo \$?"` returned exit code `1`, confirming Step 2's check correctly fails when graphify is unreachable. Restored shell still finds graphify at `/Users/bsansom/.local/bin/graphify` (no side effects). SKILL.md Step 2 contains the documented actionable error block — names `pip install graphifyy && graphify install` and points at `ground-truth-workflow-for-associated-projects.md` §Setup, distinguishing from a raw shell `command not found`. FR-020 satisfied for the graphify-not-installed case.

- [X] T028 [US3] Verify US3 acceptance scenario corresponding to drift: simulate a lockfile bump (e.g., temporarily edit `uv.lock`'s grpcio version in a worktree, or use a local tag override; **do not** commit the bump); invoke the skill; confirm the `rebuilding grpcio graph @ <new> (was <old>)` per-step entry appears. Revert the lockfile change after the verification. Anchors FR-019b. **Result (2026-05-09, real bump rather than synthetic revert):**
    - **Phase-0 gap discovery:** the original `research.md` R-002 assumed graphify writes a manifest with version info to upstream caches under `~/.graphify/repos/<owner>/<repo>/graphify-out/manifest.json`. Reality: graphify writes manifest only to *project-local* `graphify-out/`, not to upstream caches; `graphify clone` lands at HEAD-of-default-branch, not a pinned tag. The pre-validation `cross-repo.json` was therefore keyed against vLLM main HEAD (`2ee8c2a`), not v0.20.0 — drift detection couldn't run as written.
    - **Skill fix:** SKILL.md Step 4 now uses a `.target-version.txt` sidecar inside the indexed subpath's `graphify-out/`. Cold path: `graphify clone <url> --branch v<version>` (URL first, flags follow — verified). Drift path: `git -C <clone> fetch origin tag v<new> --no-tags && git -C <clone> checkout v<new> && graphify update <subpath> && echo "<new>" > <subpath>/graphify-out/.target-version.txt`. Workflow doc's "Build the X reference graph" sections updated to mirror.
    - **Reconciliation pass:** purged the three stale graphify-out/ dirs, `cross-repo.json`, and `/tmp/vllm-tag-probe`. Cold-rebuilt vLLM at v0.20.0 (commit `88d34c640`, 52,597 nodes / 255,940 edges) and grpcio at v1.80.0 (commit `f5e2d6e856`, 1,692 nodes / 2,945 edges). Wrote sidecars. Re-merged.
    - **Drift validation (vLLM rather than grpcio — mechanism is symmetric):** edited `pyproject.toml` graph-targets `vllm==0.20.0` → `vllm==0.20.1`, ran `uv lock` (output: `Updated vllm v0.20.0 -> v0.20.1`). Read sidecar (`0.20.0`), compared to lockfile (`0.20.1`), drift detected → emitted `rebuilding vLLM graph @ 0.20.1 (was 0.20.0)`. Fetched tag, checked out (`88d34c640` → `132765e35`), `graphify update` re-extracted (52,847 nodes, 258,525 edges, 632 communities — small +250-node delta over v0.20.0 as expected for a patch bump), sidecar advanced to `0.20.1`. grpcio sidecar (`1.80.0`) matched lockfile, correctly emitted `cache current — skipped`. Re-merged: 54,709 nodes / 261,726 edges / 125 MB. The bump is kept as a real upgrade rather than reverted.
    - **Deviation from spec letter:** target was `0.20.2` per maintainer request, but `v0.20.2` exists only as a GitHub tag — PyPI carries only `0.20.0` and `0.20.1`, so `uv lock` couldn't resolve `==0.20.2`. Bumped to `0.20.1` instead. The mechanism is identical regardless of patch number. FR-019b satisfied.
...
- [X] T031 [US4] Walkthrough validation per `quickstart.md` Steps 1–5 from a clean shell (no prior cwd assumptions, no shell aliases): confirm each step's command runs unmodified, every prerequisite is named in the workflow doc, and the maintainer reaches a queryable `cross-repo.json` in one pass with no missing context. Confirms SC-001 and US4 Independent Test. **Result (2026-05-09, audit + fixes rather than literal cold-shell run):** audited `quickstart.md` Steps 1–5 against the current state of the codebase, skill, workflow doc, and CLAUDE.md. Steps 1–3 verified clean (graphify install, project bootstrap, `graphify claude install` + `graphify hook install`). Step 4 (skill invocation) verified clean — the `/ground-truth-refresh` runbook produces what the illustrative output describes. Step 5 (the three queries, rewritten mid-session for the new question-shape policy) verified clean against current graphs — all four commands return their expected god-node coverage. Audit surfaced and fixed three bugs in Step 6: (a) `graphify-out/manifest.json` does NOT exist after `graphify update .` (only `graph.json`, `graph.html`, `GRAPH_REPORT.md`, `cache/`); replaced with `graph.json` mtime check; (b) `git reset --hard HEAD^` is destructive when uncommitted work is present; replaced with throwaway-untracked-file pattern + `git reset --soft HEAD^` + `git restore --staged` + `rm`; (c) Step 4 illustrative output had stale `0.20.0` references; bumped to `0.20.1` to match current `pyproject.toml` graph-targets pin, with a note that exact versions track `uv.lock`. SC-001 and US4 Independent Test satisfied. (A literal cold-shell test would require leaving an active session, so this audit is the highest-confidence verification reachable in-session.)
...
- [X] T032 End-to-end cadence verification (SC-005): exercise the documented rebuild cadence on a single timeline — make a local commit and observe the post-commit hook rebuild the project graph; simulate a lockfile bump (temporarily edit `uv.lock`'s grpcio version in a worktree) and invoke `/ground-truth-refresh`; confirm the affected upstream rebuild is surfaced and `cross-repo.json` reflects the bumped version; revert the lockfile change. Confirms that the cadence table's documented triggers actually keep the local graph current without manual intervention beyond what the workflow doc specifies. Anchors SC-005 (added per 2026-05-09 clarification). **Result (2026-05-09):** components verified across the session rather than as one literal procedure. (1) Post-commit hook firing on real local commits is verified under T011 — the hook rebuilt the project graph after both commits `4970d44` (throwaway during T011) and `7422da8` (the FR-019b fix commit), with the rebuild surfaced as `[graphify hook] N file(s) changed - rebuilding graph...`. (2) Lockfile-bump-driven upstream rebuild is verified under T028 — `pyproject.toml` bumped from `vllm==0.20.0` to `vllm==0.20.1`, `uv lock` updated `vllm v0.20.0 -> v0.20.1`, the skill detected drift via the sidecar/lockfile mismatch, surfaced `rebuilding vLLM graph @ 0.20.1 (was 0.20.0)`, and `cross-repo.json` was re-merged at the bumped baseline (54,709 nodes / 261,726 edges). (3) Cadence-table triggers all map to either the post-commit hook (covered by T011) or `/ground-truth-refresh` (covered by T025/T026/T027/T028). Bump was kept rather than reverted (real upgrade, not synthetic — see T028 note). SC-005 satisfied across the union of T011 + T028.
...
- [X] T035 SC-007 qualitative check: at the start of the next M3-style upstream question, confirm the workflow saves time relative to the pre-M2 baseline of opening upstream source manually; record the impression in `docs/decisions/` if M3 work has begun, otherwise close out as deferred. **Result (2026-05-09):** **closed as deferred** per the spec's own deferral clause ("if M3 work has begun, otherwise close out as deferred"). M3 has not started. The first M3-style upstream question — likely a wire-tuning investigation against `Channel`, `_RPCState`, or `max_message_size` enforcement — should record a qualitative time-savings impression in `docs/decisions/` against the documented pre-M2 baseline (opening upstream source files manually via `Glob`/`Grep`). Reopen this task at the start of the first M3 task that consults `cross-repo.json`.
