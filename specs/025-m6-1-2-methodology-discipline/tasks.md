...
- [X] T002 Verify `tcptraceroute` (Michael Toren's binary) is installed and on PATH per FR-002 + round-2 Q3 (`tcptraceroute --version` returns a version string; if absent, install via `brew install tcptraceroute` on macOS or `apt install tcptraceroute` / `dnf install tcptraceroute` on Linux per [`quickstart.md`](./quickstart.md) Phase 0). **Done 2026-05-17**: tcptraceroute 1.5beta7 installed via Homebrew at `/opt/homebrew/bin/tcptraceroute`; macOS Homebrew setuid fixup applied (`chown root:wheel + chmod u+s` per [`quickstart.md`](./quickstart.md) Phase 0); unprivileged probe to `1.1.1.1:443` reaches destination with `[open]` SYN-ACK in 8 hops.
...
- [ ] T029 Create `tools/benchmark/src/vllm_grpc_bench/m6_1_2_sweep.py` orchestrator (per [`data-model.md`](./data-model.md) "Cohort iteration semantics"): `run_m6_1_2_sweep(args: argparse.Namespace) -> M6_1_2SweepArtifact` (U1 remediation: `args` namespace consistent with T030's signature; no separate `M6_1_2Config` dataclass). Sequence per R-8: (1) Modal deploy + handshake (reuses `provide_m6_endpoint` / `provide_m6_1_rpc_driver` UNCHANGED per FR-022), (2) call `m6_1_2_network_probe.run_topology_probe(handshake_dict, cohorts=M6_1_2_COHORTS, per_cohort_timeout_seconds=30)` per FR-001, (3) iterate `M6_1_CELLS` (REUSED per R-2; from `m6_1_types.py:72-82`) — for each `(path, _, c)` cell, call `cohorts_at_concurrency(c)` (per T021), then for each cohort run warmup then measurement via `m6_1_rpc_driver.py`'s dispatch (T020's 4-cohort table), (4) call `m6_1_2_reporter.write_m6_1_2_report(...)`. Top of file carries the `_stderr_ts()` helper per T026; every `print(..., file=sys.stderr, flush=True)` call prefixed. Inherits M6.0a-corrected concurrent dispatch from `m6_1_1_sweep.py`'s `_measure_cell` pattern (per FR-022 — no engine-cost / dispatch-mechanics changes).
- [ ] T030 [P] Create `tools/benchmark/src/vllm_grpc_bench/m6_1_2_validate.py` per [`data-model.md`](./data-model.md) "Module Surface Map" — **the single CLI entry module for BOTH `--m6_1_2` and `--m6_1_2-validate` mode flags** (C1 remediation: per FR-024 the two flags have identical sweep shape, so one entry function handles both; the mode distinction is operator-intent metadata recorded in `run_meta.sweep_mode`, not a separate code path). Function signature: `run_m6_1_2(args: argparse.Namespace, *, sweep_mode: Literal["full", "validate"]) -> int`. Invokes `run_m6_1_2_sweep(...)` from T029 with `n_measurement=50`, `M6_1_CELLS` (the full M6.1.1 6-cell matrix per FR-024 + round-1 Q4), `M6_1_2_COHORTS` (4-element); passes `sweep_mode` through to the orchestrator so the reporter (T028) records it in `run_meta.sweep_mode`; writes the artifact to the canonical path `docs/benchmarks/m6_1_2-methodology-discipline.{md,json}` per FR-029 (overridable via `args.m6_1_2_report_out` / `args.m6_1_2_report_json_out`); returns 0 on success, non-zero per the exit-code table in [`contracts/cli.md`](./contracts/cli.md).
- [ ] ~~T031~~ **MERGED INTO T030** per `/speckit-analyze` finding C1 (HIGH — Constitution III Phase Discipline). The original T031 proposed a separate `m6_1_2_full.py` module for the `--m6_1_2` flag, but per FR-024 the two flags ship the same sweep shape — a parallel module would be a speculative abstraction (Constitution III prohibits "code written 'because we'll need it later'"). Both flags dispatch to `run_m6_1_2(args, sweep_mode=...)` in `m6_1_2_validate.py` (T030); the mode distinction lives in metadata, not code. This change reduces the new-module count from 6 back to 5 (matching plan.md / data-model.md / `contracts/cli.md` authorization).
- [ ] T032 Patch `tools/benchmark/src/vllm_grpc_bench/__main__.py` (lines 525-600 area, mirroring M6.1.1's argparse block): add `--m6_1_2` (action `store_true`), `--m6_1_2-validate` (action `store_true`), and the 12 namespaced sub-flags per [`contracts/cli.md`](./contracts/cli.md) with the EXACT defaults shown there: `--m6_1_2-modal-region` defaults to `"eu-west-1"`, `--m6_1_2-base-seed` defaults to `42`, `--m6_1_2-model` defaults to `"Qwen/Qwen3-8B"` (verbatim from M6.1.1 per FR-027 + round-3 Q2). Add `--m6_1_2` and `--m6_1_2-validate` to the mutual-exclusion list against all 13 prior mode flags listed in FR-026. Dispatch wiring (C1 remediation — single entry function): `if args.m6_1_2: return run_m6_1_2(args, sweep_mode="full"); if args.m6_1_2_validate: return run_m6_1_2(args, sweep_mode="validate")` — both flags route to the same `run_m6_1_2(...)` function in `m6_1_2_validate.py` (T030), distinguished only by the `sweep_mode` metadata argument. Also explicitly verify the existing `--m6_1_1-diagnose` argparse block is UNTOUCHED by this patch (FR-028 G1 remediation: T033 adds a regression test that asserts the M6.1.1 flag still parses identically post-M6.1.2).
...
- [ ] T033a [P] Add to `tools/benchmark/tests/test_m6_1_2_cli.py` the **FR-028 regression test** (G1 remediation from `/speckit-analyze`): `test_m6_1_1_diagnose_unchanged_post_m6_1_2()` parses `["--m6_1_1-diagnose"]` and asserts (a) it succeeds without argparse error (no new mutual-exclusion conflict accidentally introduced), (b) the resulting Namespace contains `args.m6_1_1_diagnose is True`, (c) the M6.1.1 inheritable defaults survive: `args.m6_1_1_modal_region == "eu-west-1"`, `args.m6_1_1_base_seed == 42`, `args.m6_1_1_model == "Qwen/Qwen3-8B"` (sourced from `__main__.py:541/563/568`). If a future refactor silently drifts the M6.1.1 argparse block — e.g., a planner accidentally renames `--m6_1_1-modal-region` while adding `--m6_1_2-modal-region` — this test fails and blocks merge. Closes the FR-028 coverage gap surfaced by `/speckit-analyze` G1 (MEDIUM).
...
- [ ] T042a **Pre-push grep verification (G2 remediation from `/speckit-analyze`)** — assert M6.1.2 didn't accidentally touch out-of-scope directories per FR-022 + FR-023 (process constraints with no other enforcement mechanism). Run: `git diff --name-only main..HEAD -- proxy/ frontend/ client/ proto/ scripts/python/modal_bench_rest_grpc_server.py` and assert the output is empty (no files modified in those paths). Then run: `git diff --name-only main..HEAD -- 'docs/benchmarks/m6_*.md' 'docs/benchmarks/m6_*.json'` and assert the only modifications are NEW files matching `docs/benchmarks/m6_1_2-methodology-discipline.{md,json}` (FR-029 canonical path) — NO modifications to M6 / M6.1 / M6.1.1 / M6.0a prior artifact bodies per FR-023. If either check fails, halt before push: an out-of-scope edit either violates FR-022 / FR-023 or needs explicit spec / plan update before proceeding. ANALYSIS.md is permitted to be modified (per FR-008 T040) and is NOT covered by these checks.
...
Phase 4 (US2, T017–T023) ──────┤── US3 (T024–T027) is independent and may run in parallel
   ...
   │                            │
   ▼                            ▼
Phase 6 (Polish, T028–T043 with T031 MERGED INTO T030; T033a NEW; T042a NEW)
   │   T028 reporter depends on US1 (network_paths) + US2 (cohort_set / cohort_omissions)
   │   T029 sweep orchestrator depends on US1 + US2 + US3
   │   T030 validate.py (single CLI entry for BOTH mode flags) depends on T029
   │   T032 CLI wiring depends on T030 only (T031 was merged into T030 per C1)
   │   T033 CLI tests depend on T032
   │   T033a FR-028 M6.1.1-frozen regression test depends on T032
   │   T034 integration test depends on T032
   ...
   │   T042 final lint chain depends on T040–T041 landing in the same commit
   │   T042a pre-push grep verification depends on T042
   │   T043 push depends on T042a passing
```
...
- **Phase 6**: T028 + T030 are `[P]` (different new files; T031 was merged into T030 per C1); T033 + T033a + T034 are `[P]` (different test functions or files); T041 is `[P]` against T040 (different doc files).
...
**Format check** (per the protocol's "Format Validation" requirement; post-`/speckit-analyze` remediation):
- 44 active tasks (T031 marked MERGED INTO T030 per C1 — kept as a strikethrough entry for the analysis paper trail; effective count is 43 + T033a + T042a − T031 = 44).
- All tasks use the strict `- [ ] [TaskID] [P?] [Story?] Description with file path` format.
...
- Polish phase (T028–T043 + T033a + T042a): no Story label (correct — these are cross-cutting).
- Every task description names a concrete file path (or paths) where the edit lands.

**Post-`/speckit-analyze` remediation summary** (changes applied to address findings from the read-only consistency check):
- **C1 (HIGH — Constitution III)**: T031 merged into T030. Module count back to plan-authorized 5 new files. Both `--m6_1_2` and `--m6_1_2-validate` flags dispatch to one `run_m6_1_2(args, *, sweep_mode)` function in `m6_1_2_validate.py`; mode lives in artifact metadata, not in parallel code paths.
- **G1 (MEDIUM)**: New task T033a adds the FR-028 M6.1.1-frozen regression test. CI now fails loudly if a future refactor silently drifts the M6.1.1 argparse block.
- **U1 (LOW)**: T029 signature changed from `config: M6_1_2Config` to `args: argparse.Namespace` (consistent with T030; no separate Config dataclass needed).
- **G2 (LOW)**: New task T042a adds a pre-push grep verification that out-of-scope directories (`proxy/`, `frontend/`, `client/`, `proto/`, `scripts/python/modal_bench_rest_grpc_server.py`) were not modified and that no prior-milestone artifact bodies were touched. Enforces FR-022 + FR-023 mechanically.
