...
- [ ] T032 [US3] Verify section presence by running `grep -n "^## Repo housekeeping$" ANALYSIS.md`; expect exactly one match at a high line number (near end of file).
...
- [ ] T038 [P] Run a final pre-push verification of the entire branch: `make lint typecheck test` exit code 0 AND M6.2 fake-backed smoke exit code 0 AND `git status` clean. (Last-chance FR-013 / FR-014 / SC-005 / SC-006 gate.)
