...
- **SC-002**: After merge, the total tracked size of `docs/benchmarks/`, `tests/integration/`, `scripts/python/`, `scripts/setup/`, plus the root-level `M6_2-ANALYSIS-FRAMING-DRAFT.md` decreases by **at least 2.5 MB** relative to the pre-cleanup baseline at `fea31c0`. (The PLAN.md estimate is "~3.5 MB"; the SC-002 floor is intentionally well below to absorb cross-platform `du` rounding variance — Linux and macOS report block-aligned sizes differently, and the headline ~3.5 MB claim is the user-facing metric, not this floor.)
