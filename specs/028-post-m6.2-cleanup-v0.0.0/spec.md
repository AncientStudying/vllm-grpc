...
- Q: With the FR-003 exception retaining 9 JSONs (~936 KB), the measured size reduction drops to ~1.86 MB — below SC-002's original ≥2.5 MB floor. How should SC-002 be recalibrated? → A: Switch SC-002 to a file-count metric (file cleanup is the primary value of the beat; disk-size reduction is incidental). See Q5 below for the final SC-002 reframe (the initial answer here assumed a 13-file `logs/` un-tracking that turned out to be only 1; Q5 corrects).
- Q: SC-002 implementation revealed two count-projection errors: (a) only 1 `logs/` file (`m4-full.current`) was ever tracked, not 13 as the data-model assumed (the 11 `m4-full-<ts>.log` files and `m4-full.pid` were on-disk-only, never committed); (b) the branch added 7 `specs/028-*` artifacts that count in `git ls-files | wc -l` against the cleanup. Measured net delta vs `fea31c0`: -39 files (below ≥50). How should SC-002 be reframed? → A: Reframe SC-002 to count **cleanup-commit deletions** (file-index removals attributable to the v0.0.0 cleanup commits), not net `git ls-files` delta. New baseline: state of branch just before the v0.0.0(1/5) commit lands. New floor: ≥40 cleanup-commit deletions. Measured: 47 (45 in Commit 1 + 1 in Commit 2 + 1 in Commit 3). Verification: `git diff --diff-filter=D --name-only <pre-cleanup>..HEAD | wc -l ≥ 40`. Headline framing: "47 milestone-era files removed from the index by the v0.0.0 cleanup commits". Also: `logs/m4-full.current` recoverability is via `milestone/m5-cross-host` through `milestone/m6.2-token-budget` (the file was first tracked after M4 closed), NOT `milestone/m4-time-axis` as earlier artifacts incorrectly stated.

## User Scenarios & Testing *(mandatory)*

### User Story 1 - Operator finds a clean working tree on `main` (Priority: P1)

A maintainer (or returning contributor) clones the repo or pulls `main` and looks at `docs/benchmarks/`, `tests/integration/`, `scripts/python/`, and `scripts/setup/`. Today these directories contain a chronological accumulation of every milestone's outputs and one-shot tools (~59 tracked files comprising `phase-*` / `m3-*` / `m4-*` / `m5*-` / `m6-*` / `m6_1*-` / `m6_1_1*-` / `m6_1_2*-` / `m6_1_3*-` artifact pairs, M4-era smoke tests, the M5 supersede reprocessor, the long-defunct Phase 2 env script, plus a root-level `M6_2-ANALYSIS-FRAMING-DRAFT.md` whose content is already merged into `ANALYSIS.md`). After this story ships, the working tree retains: (a) the current milestone's artifacts (`m6_2-*`), (b) the 9 baseline-chain JSONs that `tools/benchmark/src/` reads at runtime per the FR-003 exception (`m3-channel-tuning-time.json`, `m4-time-axis-tuning.json`, `m5-cross-host-validation.json`, `m5_1-rest-vs-grpc.json`, `m5_2-transport-vs-tuning.json`, `m6-real-engine-mini-validation.json`, `m6_1-real-prompt-embeds.json`, `m6_1_1-engine-cost-instrumentation.json`, `m6_1_3-attribution-closure.json`), and (c) the still-active integration tests (`test_grpc_client.py`, `test_chat_bridge.py`, `test_completions_bridge.py`, plus their `conftest.py` / `fake_frontend.py` fixtures).

**Why this priority**: This is the entire reason `v0.0.0` exists. Without it, the diff surface for M7 and M8 includes ~59 tracked files of milestone-era noise that is irrelevant to forward work but still appears in every `git status`, every IDE file-tree, every `grep`. The two larger follow-ups (`v0.0.1` bench-harness refactor and `v0.1.0` PyPI release) both assume `v0.0.0` has shipped first.
...
- **SC-002**: The v0.0.0 cleanup commits (the five `v0.0.0(N/5)` commits) MUST collectively delete **at least 40 files** from the git index. Per the 2026-05-26 Q5 clarification, the metric is "cleanup-commit deletions" rather than net `git ls-files` delta vs `fea31c0` (which would also include the 7 `specs/028-*` spec-artifact additions that the spec-kit cycle requires). Measured: 47 cleanup-commit deletions (45 in Commit 1 + 1 in Commit 2 + 1 in Commit 3). Verification: `git diff --diff-filter=D --name-only <commit-before-v0.0.0-1-of-5>..HEAD | wc -l ≥ 40`. Headline framing: "47 milestone-era files removed from the index by the cleanup commits; 9 baseline-chain JSONs retained per FR-003 exception".
