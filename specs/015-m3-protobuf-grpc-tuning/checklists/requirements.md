...
- **Scope change after initial draft**: The hygiene task originally drafted as US3 / FR-011 / SC-006 (untrack and gitignore `graphify-out/GRAPH_REPORT.md` and `graphify-out/graph.html`) was removed from M3 after investigation showed it had already landed in M2 (PR #16, task T034): `git ls-files graphify-out/` is empty on `main` and `/graphify-out/` is already in `.gitignore`. The friction the user described surfaced only because their `main` checkout pre-dated the M2 merge. The M3 spec retains a brief Note recording this for traceability.
