...
- **FR-019**: Code names de-prefix, but the harness's **data pointers** stay milestone-named — because v0.0.1 is a structural refactor that preserves *what the harness operates on*, and the harness is currently configured for the delivered M6.2 milestone. Re-targeting the harness to a new milestone's artifacts is research work (M7), explicitly out of v0.0.1's scope. Concretely, three classes of milestone-named path MUST be preserved verbatim: (a) **baseline inputs** the sweep reads (e.g. the default `docs/benchmarks/m6_1_3-attribution-closure.json` anchor and the rest of the M3→M6.1.x chain) — these are *other* delivered milestones' frozen outputs and are not ours to rename; (b) the **published canonical deliverable** `docs/benchmarks/m6_2-token-budget.{json,md}` on disk; (c) the **hardcoded canonical/validate path constants** in the validate module (e.g. `_CANONICAL_JSON = "docs/benchmarks/m6_2-token-budget.json"`, `_VALIDATE_JSON = "…-validate.json"`) — `m6_2_validate` *compares against* the published canonical file, so de-prefixing the constant would point it at a nonexistent file. Implementers MUST NOT "tidy" any of these to match the now-generic code names; doing so orphans the retained baseline-chain artifacts and breaks `m6_2_validate` and `ANALYSIS.md` references. A fourth class is added by FR-020: (d) the **per-milestone benchmark report writeups** that `ANALYSIS.md` hyperlinks.
- **FR-020**: The per-milestone benchmark report writeups under `docs/benchmarks/` that `ANALYSIS.md` hyperlinks (`m3-channel-tuning{,-time}.md`, `m4-time-axis-tuning.md`, `m5-cross-host-validation.md`, `m5_1-rest-vs-grpc.md`, `m5_2-transport-vs-tuning.md`, `m6-real-engine-mini-validation.md`, `m6_0a-dispatch-correction.md`, `m6_1-real-prompt-embeds.md`, `m6_1_1-audit-2026-05-16-seq-dispatch.md`, `m6_1_1-engine-cost-instrumentation.md`, `m6_1_2-methodology-discipline.md`, `m6_1_3-attribution-closure{,-validate}.md`) and their `.json` data siblings MUST exist on disk so that every `ANALYSIS.md` benchmark-artifact hyperlink — including in-document `#anchor` links into those reports — resolves to a live file (SC-010c). **Background:** a prior cleanup (`v0.0.0`) erroneously deleted these report writeups (and two `.json` data files, `m3-channel-tuning.json` / `m6_1_2-methodology-discipline.json`) while leaving `ANALYSIS.md`'s hyperlinks to them in place — a documented owner oversight. v0.0.1 restores them verbatim from the `milestone/m6.2-token-budget` tag (the last tag carrying the complete set before the deletion), which is the same content their `milestone/m*` recovery path returns (FR-014), so restoration introduces no narrative drift. The deliberately-removed `summary.md` redirect stub is **excluded** — `ANALYSIS.md` references it only in prose/code-span (never as a hyperlink) and explicitly records it as folded into §M1/§M3 and recoverable via tag; restoring it would contradict that decision.
...
- **SC-003**: Exactly one chat-prompt builder exists in the harness and exactly one report-generation module (`reporter.py`) exists; `CohortKind` carries exactly the 4 live members. **Symbol-level interpretation (per the T028a decision, approved 2026-05-30 — see ADR 0008):** milestone-agnosticism extends to *public symbol names*, not just module names/imports — no public symbol in the harness carries a milestone prefix (`M6_2*`/`M6_1*`/`M5_2*`/…). T028a executes the rename; this interpretation is the gate.
...
- **SC-010**: The milestone-named data pointers resolve after the refactor: (a) the published canonical deliverable `docs/benchmarks/m6_2-token-budget.{json,md}` and every baseline-chain input the sweep defaults to (e.g. `m6_1_3-attribution-closure.json`) still exist on disk; (b) the de-prefixed `validate` smoke resolves its hardcoded canonical path against that existing published file and runs to completion, writing its own output only to the `-validate.{json,md}` paths (never overwriting the canonical deliverable); (c) every `ANALYSIS.md` benchmark-artifact **hyperlink** resolves to an existing file — including the per-milestone report writeups and `#anchor` links into them, which v0.0.1 restored from the `milestone/m6.2-token-budget` tag after the `v0.0.0` cleanup erroneously deleted them (FR-020). The only non-resolving mention is the deliberately-removed `summary.md` stub, which `ANALYSIS.md` references in prose/code-span — never as a hyperlink — and documents as folded-in + tag-recoverable.
