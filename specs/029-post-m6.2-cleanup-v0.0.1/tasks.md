...
- [ ] T015pre [US1] **PLAN GAP found in T011 — resolve before T019:** `m6_2_prompt_source.py` and `m6_2_types.py` are live `m6_2_*` modules imported by `sweep.py` (and others) but were NOT scheduled for de-prefix in Entity 2 / T009–T018. De-prefix `m6_2_prompt_source`→`$SRC/prompt_source.py` (or merge into `prompts.py` per the T007 note) and `m6_2_types`→a home for the `M6_2*` symbols (`M6_2SweepMode`, `M6_2MeasurementPoint`, `M6_2AnchorLatencySnapshot`, `M6_2_MAX_TOKENS_AXIS`, …) — either fold into the `types` home or de-prefix to `$SRC/sweep_types.py`. Decide placement with the user (mirrors the T010a/T011 fold-in reviews). Repoint all importers. Required for the T019 zero-`m[0-9]`-imports gate. Gate green.
