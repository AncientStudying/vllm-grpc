...
- [ ] T010 [P] [US1] Repoint `$SRC/modal_endpoint.py`, `$SRC/rtt_probe.py`, `$SRC/ttft.py` at `types`. Gate green.
- [ ] T010a [US1] Repoint `$SRC/symmetric_prompts.py` at the 4-member `CohortKind` AND **strip its dead `tuned_grpc_channels`/`tuned_grpc` branches** (lines ~209/230/321/336 — vestigial M5.2 5-cohort logic, dead in the 4-cohort forward harness; clarify session-3 decision). Update `test_m6_1_3_symmetric_prompts.py` and any cohort-pair assertions accordingly. Gate green.
