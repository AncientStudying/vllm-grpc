...
- [ ] T012 [US1] De-prefix `m6_2_rpc_driver`→`$SRC/rpc_driver.py`: rename, repoint at homes, and absorb the live helpers from `m6_rpc_driver` (`_build_chat_grpc_request`, `_build_chat_rest_payload`, `_rest_rtt_probe`), `m6_1_rpc_driver` (`_build_embed_grpc_request`, `_build_embed_rest_payload_m6_1`, `_normalize_rest_url_for_httpx`, `_resolve_rpc_index`), and `m6_engine_cost` (`parse_grpc_trailing_metadata`, `parse_rest_response`). Gate green.
- [ ] T013 [US1] De-prefix `m6_engine_cost`→`$SRC/engine_cost.py` (or merge into `metrics.py` if cohesive — data-model Entity 2/3) for any cost code not folded into `rpc_driver`. Gate green.
...
- [ ] T021 [US2] `git rm` all legacy test files: `$TST/{test_m3_*,test_m4_*,test_m5*,test_m6_*,test_m6_1*}.py` (NOT `test_m6_2_*`), plus the milestone-bound non-prefixed tests `test_modal_endpoint_m5_2.py` (tests deleted M5.2 path). Gate green.
- [ ] T022 [US2] Rename the 17 `test_m6_2_*.py` → generic names matching de-prefixed modules (`test_m6_2_sweep`→`test_sweep`, `test_m6_2_rpc_driver`→… , `test_m6_2_validate_cli`→`test_validate_cli`, `test_m6_2_cli`→`test_cli`, `test_monitor_m6_2_sweep_artifact_present`→`test_monitor_sweep_artifact_present`, …); repoint their imports at the de-prefixed modules/homes. Gate green.
...
- [ ] T025 [US3] Add the "Bench-harness refactor (v0.0.1)" subsection to `ANALYSIS.md` (FR-015): summarize the refactor, the BC breaks (renamed modules, unified prompt format, dropped `tuned_grpc_channels`/`tuned_grpc` cohort members, flat CLI), and the `milestone/m*` tag recovery path.
...
- [ ] T029 [P] Evaluate the SC-002 thin-module merge candidates (`symmetric_prompts`→`prompts`, `ttft`→`metrics`, `engine_cost`→`metrics`) and merge **only** where cohesion improves (no contrived merges — SC-002 directional). Gate green.
- [ ] T030 Final full gate (FR-012): `cd tools/benchmark && make lint typecheck test` green with no new suppressions vs the T001 baseline; `python -m vllm_grpc_bench --validate --skip-deploy` completes; run `graphify update .` from repo root (constitution Development Workflow).
